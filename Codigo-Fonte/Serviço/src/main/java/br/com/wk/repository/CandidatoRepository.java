...
<<<<<<< HEAD
	  @Query(value = "select count(c.*) from candidato c group by c.sexo", nativeQuery = true)
		    List<BigInteger> contaCandidatosSexo();
	  ...
	  @Query(value = "select avg(extract(year FROM age(current_date, to_date(c.data_nasc, 'DD/MM/YYYY')))) from candidato c group by c.tipo_sanguineo", nativeQuery = true)
=======
	  @Query(value = "select count(c) from Candidato c group by c.sexo")
		    List<Long> contaCandidatosSexo();
	  
	  @Query(value = "select count(c) from Candidato c where (c.peso/(c.altura*c.altura)) > 30 group by c.sexo")
	    	List<Long> contaCandidatosSexoImc();
